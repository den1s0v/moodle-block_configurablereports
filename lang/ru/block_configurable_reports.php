...
$string['emptybehavior_help'] = 'Как обрабатывать фильтр, если пользователь не указал значение на странице отчёта.

* **Убрать условие** — условие в SQL не применяется (обычное поведение плагина; выборка может быть очень большой).
* **Считать ложным** — в SQL добавляется AND 1=0, пока пользователь не введёт значение.
* **Подставлять значение по умолчанию** — подставляется значение из поля ниже в SQL и в форму отчёта, если поле пустое. Если пользователь ввёл своё значение, используется оно.';
$string['emptybehavior_intro'] = 'Специальная настройка: влияет на SQL, если поле фильтра на странице отчёта пустое. Подробнее — в справке (иконка «?»).';
$string['emptybehavior_omit'] = 'Убрать условие (без ограничения)';
$string['emptybehavior_false'] = 'Считать ложным (AND 1=0)';
$string['emptybehavior_default'] = 'Подставлять значение по умолчанию';
$string['emptyfilter_defaultvalue'] = 'Значение по умолчанию';
$string['emptyfilter_defaultvalue_help'] = 'Используется только при выборе **Подставлять значение по умолчанию** выше.

Укажите текст, который применяется, когда фильтр на отчёте пустой (до того как пользователь его изменит). Значение подставляется в SQL отчёта и отображается в поле фильтра на странице отчёта.

Типичные сценарии:

1. **Понятный пример** — наглядный образец (фамилия, idnumber курса, типичное значение поля), чтобы пользователям было ясно, как заполнять фильтр.
2. **Ограничение выборки** — когда полный охват без фильтра не имеет смысла или слишком тяжёл для SQL; значение по умолчанию сужает данные до разумного подмножества.

Для фильтров со списком (поле пользователя/курса) введите отображаемое значение (например, название подразделения); кодирование такое же, как при выборе из списка.';
$string['emptyfilter_defaultvalue_hint'] = 'Например: образец для заполнения фильтра или ограничение, когда полная выборка не нужна.';
